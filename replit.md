...
**Authentication:** Multi-provider authentication system supporting Replit Auth (OpenID Connect) for web, email/password for native mobile apps (iOS/Android via Capacitor), and Google/Apple OAuth social sign-in. Mobile auth uses bcryptjs password hashing with HMAC-signed token-based session persistence (tokens stored in localStorage, sent via Authorization: Bearer header). Token utility in `server/mobileToken.ts` generates/verifies HMAC-SHA256 tokens with 30-day expiry using SESSION_SECRET. Social auth uses OAuth 2.0 with dual CSRF state validation (session + secure cookie for iPad WebView compatibility), Apple ID token signature verification via JWK, and Google email verification. Social auth redirects include auth_token in URL for mobile apps. Requires GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET and APPLE_CLIENT_ID/APPLE_CLIENT_SECRET secrets for social login. Auth page available at `/auth`. Social login routes: `/api/auth/google`, `/api/auth/apple`.
...
**Live Celebration System:** HD video streaming with live chat, reactions, mute/video controls, and real-time viewer counts for remote celebration participation.
**Shared Music System:** Synchronized music playback across multiple Bluetooth/WiFi devices, collaborative playlists, and device management for celebration audio experiences.
**Virtual Reactions System:** Real-time emoji reactions, photo sharing, messaging, and interactive celebration participation with live activity feeds.
