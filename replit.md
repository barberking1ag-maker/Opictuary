...
**Memorial QR Code System:** Generates printable QR codes for tombstones and memorial cards, linking to memorial pages. Codes support tombstone upload, memorial view, and general photo/video upload. Features privacy-compliant analytics with consent-aware geolocation tracking (browser native permission prompts), smart context detection (visitor vs. family/admin), comprehensive device fingerprinting (browser, OS, device type), and complete audit trails. System enforces strict privacy: IP addresses never stored, geolocation requires explicit user consent via browser prompts, zero-coordinate handling (preserves valid 0 values for equator/prime meridian), and consent flags stored in database for compliance auditing. Analytics support patent-worthy features including smart routing based on scanner context, behavioral tracking, and heatmap-ready location data.
...
**Future Messages System:** Enables scheduling of future messages with templates, recurrence support, automated email delivery, media attachments, and a global dashboard for management. Features comprehensive delivery tracking (status, attempts, errors), beautiful HTML email templates for different event types (birthdays, graduations, weddings, etc.), support for video/image attachments in emails, and automatic retry logic for recurring messages. Email delivery uses Nodemailer with configurable SMTP settings (supports Gmail, SendGrid, custom SMTP). Requires environment variables: EMAIL_USER, EMAIL_PASSWORD, EMAIL_HOST (optional), EMAIL_PORT (optional), EMAIL_FROM (optional). When SMTP is not configured, scheduled messages are logged to console for development/testing.
